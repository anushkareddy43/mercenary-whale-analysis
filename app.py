...
if df is not None:
    ...
    plt.xlabel("Amount (Units)", fontsize=14, fontweight='bold', color='black')
    plt.ylabel("Value (USD)", fontsize=14, fontweight='bold', color='black')
    plt.title("KMeans Clustering of 50 Transactions", fontsize=16, fontweight='bold', color='black')
    ...
    st.dataframe(transaction_df.style.set_properties(**{'background-color': '#f0f9ff', 'color': 'black', 'border': '1px solid #93c5fd', 'padding': '5px'}))
    ...
    if not whales.empty:
        ...
        st.sidebar.dataframe(whales[["DateTime (UTC)", "From", "To", "Amount", "Value (USD)", "Cluster"]].style.set_properties(**{'background-color': '#f0f9ff', 'color': 'black', 'border': '1px solid #93c5fd', 'padding': '5px'}))
    else:
